...
res = 0;
if isempty(KIKS_SESSIONACTIVE) then KIKS_SESSIONACTIVE = 0;end;
if mtlb_logic(mtlb_double(KIKS_SESSIONACTIVE),"==",1) then
  // L.84: No simple equivalent, so mtlb_fprintf() is called
  mtlb_fprintf("<KiKS> Error: A KiKS session is already active.\n<KiKS> Error: Use kiks_quit if you want quit the active session.\n");
  res = -1;
  return;
end;
...
if res==(-1) then
  // !! L.92: Unknown function kiks_quit not converted, original calling sequence used
  kiks_quit;
end;
endfunction
