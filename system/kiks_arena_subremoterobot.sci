function [] = kiks_arena_subremoterobo(xpos,ypos)
...
[rows,cols] = size(mtlb_double(KIKS_RBTMASK));
...
KIKS_ARENA_COLORMASK(mtlb_imp(miny,maxy),mtlb_imp(minx,maxx)) = mtlb_s(mtlb_double(KIKS_ARENA_COLORMASK(mtlb_imp(miny,maxy),mtlb_imp(minx,maxx))),mtlb_double(KIKS_RBTMASK_COL));
endfunction
