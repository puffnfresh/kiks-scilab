...
if %nargin>1 then
  ...
  H = findobj("tag",sprintf("t_kiksnetserver_clientstats_%d",pos));
  if ~isempty(H) then
    ...
    set(findobj("tag",sprintf("t_kiksnetserver_clientstats_%d_callsign",pos)),"String",cde);
  end;
else
  ...
  if ~isempty(H) then
    ...
    set(findobj("tag",sprintf("t_kiksnetserver_clientstats_%d_callsign",pos)),"String","");
  end;
end;
endfunction
